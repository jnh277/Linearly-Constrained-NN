...
# compare with large data set and different net sizes
n_data = 6000

layer_sizes = [10, 20, 30, 50, 60, 80, 100, 120, 160, 200]

n_tests = len(layer_sizes)
...
for test in range(n_tests):
    ...
    del model_uc
    # reinitialise models
    hnodes[test] = layer_sizes[test] + math.floor(layer_sizes[test]/2.0)
    n_h1 = math.floor(layer_sizes[test])
    n_h2 = math.floor(layer_sizes[test]/2.0)
...
with torch.no_grad():
    ...
    ax.plot(hnodes.numpy(), (val_loss_save.detach()-min_loss).log().numpy())
    ax.plot(hnodes.numpy(), (val_loss_save_uc.detach()-min_loss).log().numpy(), linestyle='--')
    ax.set_ylabel('log(loss-min_loss)')
    ax.set_xlabel('number hidden nodes')
