...
train_loss_reg = np.empty([args.epochs, 1])
val_loss_reg = np.empty([args.epochs, 1])

if args.display:
    print('Training invariant NN')

for epoch in range(args.epochs):
    train_loss_reg[epoch] = train(epoch).detach().numpy()
    v_loss = eval(epoch)
    if args.scheduler == 1:
        scheduler.step(v_loss)
    else:
        scheduler.step(epoch)   # input epoch for scheduled lr, val_loss for plateau
    val_loss_reg[epoch] = v_loss.detach().numpy()
    if args.display:
        print(args.save_file, 'Invariant NN: epoch: ', epoch, 'training loss ', train_loss[epoch], 'validation loss', val_loss[epoch])
...
train_loss_uc_reg = np.empty([args.epochs, 1])
val_loss_uc_reg = np.empty([args.epochs, 1])

if args.display:
    print('Training regularised standard NN')

for epoch in range(args.epochs):
    train_loss_uc_reg[epoch] = train_uc(epoch).detach().numpy()
    v_loss = eval_uc(epoch)
    if args.scheduler == 1:
        scheduler_uc.step(v_loss)
    else:
        scheduler_uc.step(epoch)
    val_loss_uc_reg[epoch] = v_loss.detach().numpy()
    if args.display:
        print(args.save_file, 'Reg Standard NN: epoch: ', epoch, 'training loss ', train_loss_uc[epoch], 'validation loss', val_loss_uc[epoch])
...
error_uc_reg = torch.cat((v1.reshape(400) - v1_pred_uc.detach(), v2.reshape(400) - v2_pred_uc.detach()), 0)
rms_uc_reg = torch.sqrt(sum(error_uc_reg * error_uc_reg) / 800)
...
if args.save_file is not '':
    ...
    data['train_loss_reg'] = train_loss_reg
    data['val_loss_reg'] = val_loss_reg
    ...
    data['train_loss_uc_reg'] = train_loss_uc_reg
    data['val_loss_uc_reg'] = val_loss_uc_reg
    data['final_rms_error_uc_reg'] = rms_uc_reg.detach().numpy()
    data['final_rms_error_reg'] = rms_error_reg.detach().numpy()
    sio.savemat('./results/'+ args.save_file+'.mat', data)


print(rms_error.detach().numpy())
print(rms_error_reg.detach().numpy())
print(rms_uc.detach().numpy())
print(rms_uc_reg.detach().numpy())
