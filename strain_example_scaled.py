...
# model = derivnets.Strain2d_sc(nn.Sequential(nn.Linear(n_in,n_h1),nn.Tanh(),nn.Linear(n_h1,n_h2),
#                                          nn.Tanh(),nn.Linear(n_h2,n_h3),nn.Tanh(),
#                                          nn.Linear(n_h3,n_o)),scale_xx=2/(max_xx-min_xx),
#                                         scale_yy=2/(max_yy-min_yy),scale_xy=2/(max_xy-min_xy))

model = derivnets.Strain2d_sc(nn.Sequential(nn.Linear(n_in,n_h1),nn.Tanh(),nn.Linear(n_h1,n_h2),
                                         nn.Tanh(),nn.Linear(n_h2,n_h3),nn.Tanh(),
                                         nn.Linear(n_h3,n_o)),scale_xx=1,
                                        scale_yy=1,scale_xy=1)

def scale(y, max_y, min_y, shift=True):
    if shift:
        scaled = 2*(y-min_y)/(max_y-min_y) - 1
    else:
        scaled = 2*y/(max_y-min_y)
    return scaled

def unscale(scaled_y, max_y, min_y,shift=True):
    if shift:
        unscaled = (scaled_y + 1)/2*(max_y-min_y) + min_y
    else:
        unscaled = (scaled_y)/2*(max_y-min_y)
    return unscaled
...
def train(epoch):
    ...
    for x1_train, x2_train, Exx_train, Eyy_train, Exy_train in training_generator:
        ...
        # loss = (criterion(scale(Exx_train,max_xx,min_xx, shift=False), Exx)
        #         + criterion(scale(Eyy_train,max_yy,min_yy, shift=False), Eyy)
        #         + criterion(scale(Exy_train,max_xy,min_xy, shift=False), Exy)) / 3
        loss.backward()
        optimizer.step()
        total_loss += loss
        n_batches += 1
...
def eval(epoch):
    ...
    # loss = (criterion(scale(Exx_val, max_xx, min_xx, shift=False), Exx)
    #         + criterion(scale(Eyy_val, max_yy, min_yy, shift=False), Eyy)
    #         + criterion(scale(Exy_val, max_xy, min_xy, shift=False), Exy)) / 3
    return loss
...
Eyy_p = Eyy_p*p_scale
Exy_p = Exy_p*p_scale
# Exx_p = unscale(Exx_p,max_xx, min_xx, shift=False)
# Eyy_p = unscale(Exy_p,max_yy, min_yy, shift=False)
# Exy_p = unscale(Eyy_p,max_xy, min_xy, shift=False)
